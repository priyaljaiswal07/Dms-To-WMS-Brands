...
def process_hul_sales(input_file, reference_file, output_file, column_mapping=None, selected_state=None,
    ...
    # Product matching with user confirmations
    ...
    for idx, prod in enumerate(sale_order_df["product_name"]):
        ...
        if score >= 100:
            ...
            # Check user decision for partial matches
            cache_key = f"{prod}|{match}"
            if cache_key in partial_decisions:
                user_confirmed = partial_decisions[cache_key] if partial_decisions[cache_key] is not None else False
            else:
                # No decision yet, default to False (needs review)
                user_confirmed = False
    ...
    # Build batch inventory and product variants mapping
    batch_inventory = {}
    product_id_groups = {}
    ...
    for idx, row in enumerate(sale_order_df.itertuples()):
        ...
        if not pname:
            # Check if it's a low match score or no match at all
            ...
            # Allocate from available batches with variant and related product support
            products_to_use = [pname]  # Start with matched product
            current_stock = sum(b["available_stock"] for b in batch_inventory[pname])
            
            # Check for product variants if stock is insufficient
            if current_stock < qty:
                ...
                # Check user decisions for variants
                for variant in variants_to_check:
                    if variant in batch_inventory:
                        ...
                        if variant_stock > 0:
                            ...
                            if cache_key in variant_decisions and variant_decisions[cache_key]:
                                products_to_use.append(variant)
            
            # Check for related products if still insufficient
                ...
                for prod_key in batch_inventory.keys():
                    ...
                    if is_potentially_related:
                        ...
                        if rp_stock > 0:
                            ...
                            if cache_key in related_decisions and related_decisions[cache_key]:
                                products_to_use.append(prod_key)
            
            # Allocate from all selected products
            remaining_qty = qty
            batch_allocations = []
            ...
            if remaining_qty <= 0:
                ...
                for batch_id, product_id, allocated_qty in batch_allocations:
                    # Calculate selling price for this portion
                ...
                # Show info if multiple batches were used
                if len(batch_allocations) > 1:
                    batch_list = ", ".join([b[0] for b in batch_allocations])
                    if st:
                        st.info(f"✅ Order {row.order_id} split into {len(batch_allocations)} rows (batches: {batch_list}) | Product: '{pname}' | Total: {int(qty)} units")
            else:
                # Not enough stock even across all batches
    ...
    if st:
        ...
        st.markdown("### 📊 Summary:")
        st.write(f"  ✅ Valid orders (100% match): {len(valid_df['order_id'].unique()) if not valid_df.empty else 0}")
        st.write(f"  ⚠️  Partially matched orders (70-99%): {len(partial_df['order_id'].unique()) if not partial_df.empty else 0}")
        st.write(f"  ❌ Orders with errors (<70%): {len(error_df['order_id'].unique()) if not error_df.empty else 0}")
        if not sales_return_df.empty:
            st.write(f"  🔄 Sales returns (count): {len(sales_return_df)}")
        
        # Show multi-batch allocation stats
        if not valid_df.empty:
            ...
            if not split_orders.empty:
                ...
                st.write(f"  {len(split_orders)} orders split into multiple rows (total {int(total_splits)} rows)")
                st.write(f"  This helped fulfill orders when single batches had insufficient stock")
                st.write(f"  Example: Order requiring 30 units might use Batch1(20) + Batch2(10) = 2 rows")
        
        if not partial_df.empty:
            st.markdown("### ⚠️ Partial Match Summary (top reasons):")
            for reason, cnt in Counter(partial_df["partial_match_reason"]).most_common(5):
                st.write(f" - {reason}: {cnt}")
