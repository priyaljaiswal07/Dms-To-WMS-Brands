"""
Marico Processor - Interactive version that asks questions during processing
...
def process_marico_sales_interactive(input_file, reference_file, output_file, 
                                     column_mapping=None, selected_state=None,
                                     processing_cache=None, pending_question_callback=None):
    """
    Process Marico sales orders - Interactive version that asks questions during processing.
    ...
    # Column mapping (auto-detect) - Marico uses same as HUL
    ...
    # Detect GST columns for Marico: CGST Rate, SGST Rate / UTGST Rate, IGST Rate, TCS Tax
    cgst_rate_col = None
    sgst_rate_col = None
    igst_rate_col = None
    tcs_tax_col = None
    for col in df.columns:
        ...
        if "cgst" in col_lower and "rate" in col_lower:
            cgst_rate_col = col
        elif ("sgst" in col_lower or "utgst" in col_lower) and "rate" in col_lower:
            sgst_rate_col = col
        elif "igst" in col_lower and "rate" in col_lower:
            igst_rate_col = col
        elif "tcs" in col_lower and "tax" in col_lower:
            tcs_tax_col = col
    
    # Parse dates with dayfirst=True (Marico-specific)
    ...
    # Add GST columns for Marico
    if cgst_rate_col:
        sale_order_df["CGST Rate"] = pd.to_numeric(df[cgst_rate_col], errors="coerce")
    else:
        sale_order_df["CGST Rate"] = ""
    if sgst_rate_col:
        sale_order_df["SGST Rate / UTGST Rate"] = pd.to_numeric(df[sgst_rate_col], errors="coerce")
    else:
        sale_order_df["SGST Rate / UTGST Rate"] = ""
    if igst_rate_col:
        sale_order_df["IGST Rate"] = pd.to_numeric(df[igst_rate_col], errors="coerce")
    else:
        sale_order_df["IGST Rate"] = ""
    if tcs_tax_col:
        sale_order_df["TCS Tax"] = pd.to_numeric(df[tcs_tax_col], errors="coerce")
    else:
        sale_order_df["TCS Tax"] = ""
    ...
    for idx, row in enumerate(sale_order_df.itertuples()):
        ...
        base_row = {
            ...
            "CGST Rate": sale_order_df.loc[row.Index, "CGST Rate"] if "CGST Rate" in sale_order_df.columns else "",
            "SGST Rate / UTGST Rate": sale_order_df.loc[row.Index, "SGST Rate / UTGST Rate"] if "SGST Rate / UTGST Rate" in sale_order_df.columns else "",
            "IGST Rate": sale_order_df.loc[row.Index, "IGST Rate"] if "IGST Rate" in sale_order_df.columns else "",
            "TCS Tax": sale_order_df.loc[row.Index, "TCS Tax"] if "TCS Tax" in sale_order_df.columns else "",
        }
        ...
        if not pname:
            ...
            if current_stock < qty:
                ...
                # Ask user to confirm each variant (like original script)
            ...
            if total_stock < qty:
                ...
                # Ask user for EACH related product (like original script)
    ...
    top_cols = ["order_id","order_date","warehouse_name","product_id","batch_id",
                ...
                "dms_invoice","low_price_reason","CGST Rate","SGST Rate / UTGST Rate","IGST Rate","TCS Tax"]
